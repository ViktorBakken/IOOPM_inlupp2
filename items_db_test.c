...
//   ioopm_item_t *item_new = ioopm_make_item_backend(strdup("ape"), strdup("a ape"), 2);
...
static void test_remove_item()
{
  ...
  ioopm_replenish_stock(&warehouse, item, 1);
  
  ioopm_list_item(warehouse.HTn,2);
  ioopm_show_stock_items(*item);

  CU_ASSERT_EQUAL(ioopm_hash_table_size(warehouse.HTn), 2);
...
int main()
{
  ...
  if (
      (CU_add_test(my_test_suite, "Test ioopm_replenish_stock", test_replenish_stock) == NULL) ||
